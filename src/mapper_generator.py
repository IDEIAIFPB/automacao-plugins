...
class MapperGenerator:
    ...
    def __init__(self, xsd_parser: XsdParser, template_xpath_prefix: str = "/SynchroId/PedidoEnvioRPS/RPS", properties_file: str = None):
        """
    ...
    def generate_mapper(self, root_element_name: str, mapper_id: str = "auto-generated") -> etree._Element:
        """
        ...
        if not root_element:
            raise ValueError(f"Elemento raiz '{root_element_name}' não encontrado no XSD.")
        ...
        mapper_root.set("{http://www.w3.org/2001/XMLSchema-instance}noNamespaceSchemaLocation", "../../../schemas/document-mapping.xsd")
        ...
        self._add_property_for_element(properties, root_element, root_element_name, "", root_element_name)
        
        return mapper_root
    
    def _collect_id_attributes(self, element: XsdElement, element_path: str = "", parent_name: str = ""):
        """
        ...
        if element.is_complex:
            for child in element.children:
                child_path = f"{element_path}/{child.name}" if element_path else child.name
            ...
            for choice_group in element.choices:
                for choice_element in choice_group:
                    choice_path = f"{element_path}/{choice_element.name}" if element_path else choice_element.name
                    self._collect_id_attributes(choice_element, choice_path, element_name)
    
    def _add_property_for_element(self, parent_node: etree._Element, element: XsdElement, property_name: str, path_prefix: str = "", root_element_name: str = ""):
        """
            ...
            for attr in element.attributes:
                ...
                if attr.name == "Id" and self.id_variables:
                    ...
                    if var_name in self.id_variables:
                        ...
                        attr_xpath = f"{self.template_xpath_prefix}/{path_prefix}/@{attr.name}" if path_prefix else f"{self.template_xpath_prefix}/@{attr.name}"
        ...
        if element.is_complex:
            ...
            for child in element.children:
                ...
                self._add_property_for_element(properties_node, child, child.name, child_path, root_element_name or property_name)
                
            # Processar choices também
            for choice_group in element.choices:
                for choice_element in choice_group:
                    choice_path = f"{path_prefix}/{choice_element.name}" if path_prefix else choice_element.name
                    self._add_property_for_element(properties_node, choice_element, choice_element.name, choice_path, root_element_name or property_name)
        else:
            ...
            properties_key = f"{root_element_name}/{path_prefix}" if path_prefix else root_element_name
    ...
    def generate_mapper_xml(self, root_element_name: str, mapper_id: str = "auto-generated") -> str:
        """
        ...
        return etree.tostring(mapper_root, pretty_print=True, encoding="utf-8", xml_declaration=True).decode("utf-8")
    
    def save_mapper_to_file(self, output_path: str, root_element_name: str, mapper_id: str = "auto-generated") -> None:
        """
