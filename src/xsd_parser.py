...
class XsdParser:
    ...
    def _parse(self):
        ...
        try:
            ...
            self.namespace_map = {prefix: uri for prefix, uri in self.root.nsmap.items() if prefix is not None}
    ...
    def _parse_simple_types(self):
        ...
        for simple_type in self.root.xpath("//xsd:simpleType", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
            name = simple_type.get("name")
            if name:
                restrictions = simple_type.xpath(".//xsd:restriction", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"})
                if restrictions:
    ...
    def _parse_complex_types(self):
        ...
        for complex_type in self.root.xpath("//xsd:complexType", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
    ...
    def _parse_elements(self):
        ...
        for elem in self.root.xpath("//xsd:element", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
            # Ignorar elementos que são filhos de tipos complexos
            if elem.getparent() is not None and etree.QName(elem.getparent()).localname not in ["schema"]:
                continue
            ...
            if name:
                ...
                complex_type_elems = elem.xpath("./xsd:complexType", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"})
    ...
    def _process_complex_type_children(self, complex_type_elem, parent_element):
        ...
        """
        # Processar atributos
        for attr in complex_type_elem.xpath(".//xsd:attribute", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
            name = attr.get("name")
            type_name = attr.get("type")
            required = attr.get("use") == "required"
            if name and type_name:
                parent_element.attributes.append(XsdAttribute(name=name, type=type_name, required=required))
        
        # Processar elementos filhos de sequence
        for sequence in complex_type_elem.xpath(".//xsd:sequence", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
            for child in sequence.xpath("./xsd:element", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
                self._process_child_element(child, parent_element)
        
        # Processar elementos filhos de choice
        for choice in complex_type_elem.xpath(".//xsd:choice", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
            choice_elements = []
            for child in choice.xpath("./xsd:element", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"}):
    ...
    def _process_child_element(self, element_node, parent_element):
        ...
        if child_element:
            ...
            child_element.path = f"{parent_element.path}/{child_element.name}" if parent_element.path else child_element.name
    ...
    def _create_element_from_node(self, element_node):
        ...
        if name:
            ...
            # Verificar se o elemento contém uma definição de tipo complexo anônimo
            complex_type_elems = element_node.xpath("./xsd:complexType", namespaces={"xsd": "http://www.w3.org/2001/XMLSchema"})
            if complex_type_elems:
                element.is_complex = True
                self._process_complex_type_children(complex_type_elems[0], element)
